...
```{r}
p1 = ggplot(
  GBM_cv_results, aes(shrinkage, as.factor(n.trees), fill = mean_error)) +
  geom_tile()+
...
```{r best_model_GBM}
par_best <- GBM_cv_results[which.min(GBM_cv_results$mean_error),]
...
if (sample.reweight){
  ...
  tm_train_GBM <- system.time(fit_train_GBM <- train_GBM(dat_train, label_train, w = NULL, 
                                                      num.trees = par_best$n.trees,
                              depth = par_best$interaction.depth,
                              rate = par_best$shrinkage,
                              bag_f = par_best$bag.fraction,
