...
run.cv.GBM<-TRUE #run cross-validation on the training set applying GBM mod
run.train.GBM<-TRUE 
run.test.GBM<-TRUE
```
...
if(run.cv.GBM){
  ...
  for(i in 1:nrow(hyper_grid_GBM)){
    print(i)
    res_cv[i,] <- cv.function_GBM(traindf = dat_train, K, 
                              ...
                              reweight = TRUE)
  save(res_cv, file="../output/res_cv_BGM.RData")
  }
}else{
  load("../output/res_cv_BGM.RData")
}
...
```{r best_model_GBM}
par_best <- GBM_cv_results[which.min(GBM_cv_results$mean_error),] # lmbd[which.max(res_cv$mean_AUC)]
par_best <- par_best[1:5]
par_best
```
...
if (sample.reweight){
  tm_train <- system.time(fit_train_GBM <- train_GBM(dat_train, w = weight_train, 
                              num.trees = par_best$n.trees,
                              depth = par_best$interaction.depth,
                              rate = par_best$shrinkage,
                              bag_f = par_best$bag.fraction,
                              n_m = par_best$n.minobsinnode))
} else {
  tm_train <- system.time(fit_train_GBM <- train_GBM(dat_train, label_train, w = NULL, 
                                                      num.trees = par_best$n.trees,
                              depth = par_best$interaction.depth,
                              rate = par_best$shrinkage,
                              bag_f = par_best$bag.fraction,
...
if(run.test){
  ...
  tm_test <- system.time({prob_pred_GBM <- test_GBM(fit_train_GBM, dat_test)[[2]]; 
...
accu <- sum(weight_test * (label_pred_GBM == label_test)) / sum(weight_test)
#accu1 <- mean(label_pred == label_test)
tpr.fpr <- WeightedROC(prob_pred_GBM, label_test, weight_test)
auc <- WeightedAUC(tpr.fpr)


cat("The accuracy of weighted BGM model:", "is", accu*100, "%.\n")
cat("The accuracy of BGM model:", "is", accu1*100, "%.\n")
cat("The AUC of BGM  model:", "is", auc, ".\n")
