...
train_XGS <- function(data=dtrain, label=label_train, weight=NULL, params=best_params){
  module_XGS <- xgboost(data=dtrain, label=label_train, weight=sample.reweight, params=best_params, nrounds = 160) 
  return(module_XGS)
}
```

```{r test_model}
test_XGS <- function(module, test){
  module_test <- predict(module, test)
  return(module_test)
}
...
sample.rewight=FALSE
if (sample.reweight){
  tm_train_XGS <- system.time(fit_train_XGS <- train_XGS(dtrain, label_train, w=weight_train, best_params))
} else {
  tm_train_XGS <- system.time(fit_train_XGS <- train_XGS(dtrain, label_train, weight=NULL, best_params))
}
...
if(run.test){
  ...
  tm_test_XGS <- system.time({label_pred <- as.integer(test_XGS(fit_train_XGS, feature_test)); 
                            })
}
```

```{r}
accu <- mean(label_test == label_pred)
tpr.fpr <- WeightedROC(as.numeric(label_pred), label_test)
auc <- WeightedAUC(tpr.fpr)


cat("The accuracy of model is", accu*100, "%.\n")
cat("The AUC of modelis", auc, ".\n")
...
```{r}
# xgb.cv( params = list(max_depth= par_best_max_depth,
#           min_child_weight=par_best_min_child_weight,
#            subsample= par_best_subsample, 
#            colsample_bytree= par_best_colsample), 
#         data = dtrain, 
...
#         showsd = T, 
#         stratified = T, 
#         print_every_n = 10, early_stop_round = 20, maximize = F,eval_metric="auc",eval_metric="error")
...
lrn_1$par.vals <- list( objective="binary:logistic", eval_metric="error",eval_metric="auc", nrounds=200L, eta=par_best_eta,scale_pos_weight=sumwneg/sumwpos)
