...
#three other versions of xgboost:
SMOTE
Weighted(Already tuned)
PCA

don't do any cross validation, just to implementing them using the xgboost model with best tuning parameters 
```{r best setting of tunning parameter}
best_params<- list(
  max_depth = 10,
...
table(label_train.S)
#Preparing XGboost
dtrain.SMOTE<-xgb.DMatrix(feature_train.S, label=label_train.S)
#train the XGBoost+SMOTE 
xgb.SMOTE<-xgb.train(params=best_params,
                data=dtrain.SMOTE,
                nrounds=160,
          eval_metric="auc",
          eval_metric="error",
...
feature_test<-as.matrix(dat_test[,-6007])
label_test<- as.integer(dat_test$label)-1
xgb.SMOTE.pred<-predict(xgb.SMOTE,feature_test)
xgb.SMOTE.label<-ifelse(xgb.SMOTE.pred>0.5,1,0)
xgb_smote.cm<-confusionMatrix(as.factor(xgb.SMOTE.label), as.factor(label_test))
xgb_smote.cm #balanced accuracy 0.7080 
auc<-roc(label_test,xgb.SMOTE.pred)$auc # 0.829
weight_test <- rep(NA, length(label_test))
for (v in unique(label_test)){
  weight_test[label_test == v] = 0.5 * length(label_test) / length(label_test[label_test == v])
}
fr.tr<-WeightedROC(xgb.SMOTE.pred,label_test,weight_test)
 auc<-WeightedAUC(fr.tr) #weighted auc 0.829
```
