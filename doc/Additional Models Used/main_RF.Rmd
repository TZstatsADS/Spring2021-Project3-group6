---
title: "Random Forest Model for imbalanced binary classification"
author: "Weiwei Song"
output:
  pdf_document: default
---
This files contains contains two algorithms about the random forest:
One is the weighted random forest and another one is the weighted random forest. 
```{r message=FALSE}
if(!require("EBImage")){
...
Noted: use the same parameter lists to do grid search 
```{r model_setup}
##Random Forest 
...
if(run.cv.BRF){
  ...
  for(i in 1:nrow(hyper_grid_RF)){
    ...
    res_cv_BRF[i,] <- cv.function_BRF(features = feature_train, 
...
```{r best_model_BRF}
par_best_BRF <-hyper_grid_RF[which.min(result_cv_BRF$mean_error),]
...
if (run.train.BRF) {
    tm_train_BRF <- system.time({
      ...
      fit_train_BRF <- train_BRF(features = feature_train, labels = label_train,
...
if(run.test.BRF){
  load(file="../output/fit_train_BRF.RData")
  tm_test_BRF <- system.time({ prob_pred <- test_BRF(fit_train_BRF, feature_test)$predictions
...
if(run.cv.WRF){
  ...
  for(i in 1:nrow(hyper_grid_RF)){
    cat("n_tree =", hyper_grid_RF$ntree[i],
        ", mtry=", hyper_grid_RF$mtry[i],"\n")
    res_cv_WRF[i,] <- cv.function_WRF(features = feature_train, 
                              labels = label_train, K, 
                              num_tree=hyper_grid_RF$ntree[i],
                              mtry=hyper_grid_RF$mtry[i],
...
if (run.train.WRF) {
  ...
  if (sample.reweight){
    tm_train_WRF <- system.time(
      fit_train_WRF <- train_WRF(features = feature_train, labels = label_train,
                                 w =  weight_train,
                                 num_tree = par_best_ntree,
                                 mtry = par_best_mtry))
  }else{
...
if(run.test.WRF){
  load(file="../output/fit_train_WRF.RData")
  tm_test_WRF <- system.time({ prob_pred <- test_WRF(fit_train_WRF, feature_test)$predictions
                              label_pred <- ifelse(prob_pred[,1]>prob_pred[,2],1,2)})
}
```
