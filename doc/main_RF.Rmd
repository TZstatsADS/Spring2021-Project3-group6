...
run.cv.RF<-FALSE #run cross-validation on the training set applying GBM mod
run.train.RF<-FALSE 
run.test.RF<-FALSE
```
...
```{r best_model_RF}
par_best_RF <-hyper_grid_RF[which.max(result_cv_RF$mean_AUC),]
...
if(run.test.RF){
  load(file="../output/fit_train_RF.RData")
  tm_test_RF <- system.time({ prob_pred <- test(fit_train_RF, feature_test)$predictions
                              label_pred <- ifelse(prob_pred[,1]>prob_pred[,2],1,2)})
}
```
