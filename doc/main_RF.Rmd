...
)# the parameter mtry is often sensible, we only use the ntree as the tunning parameter 
dim(hyper_grid_RF)
```
...
if (sample.reweight){
 tm_train_RF <- system.time(fit_train_RF <- train(feature_train, label_train, w = weight_train, num_tree=par_best_ntree,mtry=par_best_mtry))
} else {
  tm_train_RF <- system.time(fit_train_RF <- train(feature_train, label_train,NULL,num_tree=par_best_ntree,mtry=par_best_mtry))
 }
